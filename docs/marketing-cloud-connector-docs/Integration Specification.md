...
* Installation Guide: [[Installation Specification]]
* [[MC Connector Installation - Triggered Send Configuration]]
* Repository: [Marketing Cloud Connector](https://github.com/SalesforceCommerceCloud/marketing-cloud-connector)
* MC Connector dependency: [Handler Framework](https://github.com/SalesforceCommerceCloud/handler-framework)
...
The Marketing Cloud Connector is a reusable code asset to support the enablement and acceleration of specific integration use cases between Commerce Cloud and Marketing Cloud. SiteGenesis is used as a reference storefront for MC Connector cartridge integration. Follow Installation Guide to set up MC Connector: [[Installation Specification]]

## Features & Requirements

MC Connector cartridge will have new functionality added to it in an incremental manner. 
...
MC Connector itself doesn’t have any touch-points in SiteGenesis, rather the Handler Framework that it depends on, does the SG integration.

Please, see  Handler Framework modifications to SiteGenesis for more details: [Handler Framework - Installation Guide](https://bitbucket.org/demandware/marketing-cloud-connector/wiki/Handler%20Framework%20-%20Installation%20Guide) 

### Implementation Approach
See below for a brief overview for the MC Connector implementation approach.

* CommonJS modules are used
    * one module for the triggers
    * one (or more) module(s) for the actual REST/SOAP service communication
    * no pipelines
...
### Integration cartridge: **int_marketing_cloud**

See below for the **int_marketing_cloud** cartridge file structure.
...
Transactional emails will make use of Triggered Email functionality in Marketing Cloud.

### REST APIS

We're using REST API for MC Connector implementation.
Documentation: https://developer.salesforce.com/docs/atlas.en-us.mc-apis.meta/mc-apis/index-api.htm

### Authentication

MC uses oAuth for authentication, which returns a token upon successful auth.
To avoid authenticating excessively, the retrieved token will be stored into a custom object for reuse.

**Standard API Calls**

All calls to the REST API (other than the auth call) will be modified to apply the auth header.
If a token is missing or expired, a new token will be automatically requested in this process.

### Response Handling

All standard API calls will have their response parsed, and the response object wrapped.
The object wrapper will additionally contain checks indicating if there was a JSON parsing error, such as an auth error.
...
The executing code will be responsible for reacting to any errors and handling retries.

### Email Hooks

Email sending in the application is abstracted into hooks which are called like

**Sample communication hook execution**
...
The supported trigger hooks which are initiated by user interaction are:
...
The hook interface would look like

**Sample communication hook implementation**
...
To support triggering MC emails, we'll be replacing current email logic within various locations of SiteGenesis with the MC alternative. This is only an alternative, so we still need to support standard email execution.
...
Custom jobs will be built to be executed within Business Manager.
Flat-file batch exports will be generated at minimum on a daily basis.
...
The approach used for chunk and task oriented custom job steps has followed the recommendations [documented in InfoCenter](https://documentation.demandware.com/DOC1/index.jsp?topic=%2Fcom.demandware.dochelp%2FJobs%2FCreatingCustomJobSteps.html).
Feeds and import activities will follow the documentation provided for Marketing Cloud:
...
See int_marketing_cloud/cartridge/steptypes.json


## Site Preferences

For Phase 1, this integration is not leveraging any Site Preferences. The capability to turn off MC transactional emails is available via custom object config (see Installation Guide).

Site Preferences will be used to manage some preferences related to Analytics collection in Phase 2.