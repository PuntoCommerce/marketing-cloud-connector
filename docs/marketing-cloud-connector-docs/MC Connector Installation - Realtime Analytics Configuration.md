...
**Please work with your Marketing Cloud representative to address any questions or concerns that may arise.**
Everything on this page merely exists as recommendations, intended to help you configure your account more quickly.

## Events

MC Connector's Analytics functionality supports configurable event tracking, using a field mapping approach similar to Triggers and Data Feeds.  
...
Custom events can be fired when any of the above **Available Events** occur. A custom event can be named to whatever value desired.  

**Caveat**: Custom events are sent to Marketing Cloud using collect.js's `trackEvent` method. This data may or may not be usable on Marketing Cloud side, please speak with your representative to confirm. To ensure that custom event data is always available in some form, custom event data is also added to the `setUserInfo` method. You can then work with your MC representative to ensure you're able to react to information set on the user profile.
