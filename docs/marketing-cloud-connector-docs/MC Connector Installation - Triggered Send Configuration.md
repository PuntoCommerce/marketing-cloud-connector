...
Data extensions data retention is wholly up to you.  
...
For email creation, it's recommended to use Content Builder, which you may need to opt-in to on your account before you can see it.
...
For triggers that heavily overlap in the fields that are defined/used, it's feasible to create a single data extension to be shared across multiple triggers. This approach is recommended for the Account triggers (Created, Updated, Locked, Password Changed). The Reset Password trigger may be best to be stored in it's own data extension.
